Vue.component('essaydetail', {
	template:
	`<div class="essaydetailBody">
		<div class="essayTitle">{{data.titileStr}}</div>
		<div class="essayRemark">
			<div class="essayTime">46分钟前</div>
			<div class="essayAuthor">mader</div>
		</div>
		<div class="essayContent" v-html="data.contextStr"></div>
		<div class="essayLabels">
			<div class="essayLabelItem" v-for="(item,index) in data.yiliaoLableList" :key="index">{{item.lableStr}}</div>
		</div>
	</div>`,
	props: {
		data: {
